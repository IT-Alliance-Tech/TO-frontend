...
const SubscriptionPlans = () => {
  ...
  // ✅ Get active subscription from logged-in user
      ...
      {user ? (
        activeSubscription ? (
          <div className="subscription-warning-banner success-banner">
            <i>✔</i> Active Subscription:{" "}
            <strong>{activeSubscription.subscriptionId.name}</strong> | Expires on{" "}
            {formatDate(activeSubscription.endDate)} | Slots Left:{" "}
            {activeSubscription.available} / {activeSubscription.subscriptionId.accessibleSlots}
          </div>
        ) : (
          <div className="subscription-warning-banner">
            <i>❗</i> You do not have an active subscription. Choose a plan to access property contact details.
          </div>
        )
      ) : (
      ...
      <div className="subscription-plans-container">
        {loading ? (
          ...
          plansData.map((plan, index) => {
            const themeColors = {
              gold: { text: "#DAA520", button: "linear-gradient(135deg, #FFD700, #DAA520)", shadow: "0 10px 25px rgba(218,165,32,0.35)" },
              silver: { text: "#C0C0C0", button: "linear-gradient(135deg, #C0C0C0, #A9A9A9)", shadow: "0 10px 25px rgba(128,128,128,0.35)" },
              diamond: { text: "#5CD4FC", button: "linear-gradient(135deg, #7FDBFF, #39C0ED)", shadow: "0 10px 25px rgba(60,180,220,0.35)" },
            };

            const name = plan.name?.toLowerCase();
            const theme = name.includes("gold")
              ? themeColors.gold
              : name.includes("silver")
              ? themeColors.silver
              : name.includes("diamond")
              ? themeColors.diamond
              : { text: "#1e3a8a", button: "#2563eb", shadow: "0 10px 25px rgba(0,0,0,0.15)" };
                ...
                <div className="subscription-plan-body">
                  ...
                  {isActivePlan && (
                    <div className="plan-status-box">
                      ...
                      <p>
                        ✔ Slots Left: {activeSubscription.available} / {activeSubscription.subscriptionId.accessibleSlots}
                      </p>
                    </div>
                  )}
                </div>
